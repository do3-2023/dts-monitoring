...
#### 2. Install Cilium with the config values that can be found in *kube/cilium-conf.yaml*:

```
helm repo add cilium https://helm.cilium.io/

helm install cilium cilium/cilium --version 1.15.6 --namespace kube-system -f ./cilium-conf.yaml
```

Restart all cluster pods to make Cilium start managing them:
...
#### 5. Create a gateway/gateways for your IP addresses listening on port 80

```
kubectl apply -f gateways.yaml
```

#### 6. Create three namespaces for the frontend, api and database pods respectively:
...
#### 8. Create secrets in `front` and `back` namespaces to allow Kubernetes access private Docker repository to pull container images:
...
#### 12. Launch test deployments for "green" versions of API and frontend:
...
#### 13. To update the app version with a Blue/Green deployment strategy
...
#### 14. Launch a Wasm API:

```
kubectl apply -f ./wasm
```

#### 15. Launch a new frontend to test the Wasm API:
